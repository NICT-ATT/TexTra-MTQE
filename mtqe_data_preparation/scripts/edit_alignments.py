...
def read_file(file_path):
    toreturn = []
    with open(file_path, mode = 'r', encoding = 'utf-8') as fid:
        for line in fid:
            toreturn.append(line.rstrip().split())
    return toreturn

assert len(sys.argv[:1]) == 1, "Expected one argument"

in_tercom_xml, in_mt_original, in_pe_original, out_align = sys.argv[1:]
...
for sent_index, sent_edits in enumerate(edits):
    ...
    for edit in sent_edits:
        ...
        if edit.o == 'C':
            ...
            if mt_original[sent_index][mt_original_index].lower() != pe_original[sent_index][pe_original_index].lower():
...
with open(out_align, mode = 'w', encoding = 'utf-8') as fid:
